...
void LUP_InverseMatrix(const emxArray_real_T *A, emxArray_real_T *A_)
{
  ...
  for (k = 0; k <= N; k++) {
    ...
    for (ii = 0; ii <= i0; ii++) {
      ...
      if (fabs(b_A->data[((int)unnamed_idx_1 + b_A->size[0] * k) - 1]) > delta1)
      {
        delta1 = fabs(b_A->data[((int)unnamed_idx_1 + b_A->size[0] * k) - 1]);
...
/*
 * File trailer for LUP_InverseMatrix.c
 *
 * [EOF]
 */
